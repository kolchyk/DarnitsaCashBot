...
def onboarding_text(*, require_phone: bool, user_name: str = "") -> str:
    ...
    return (
        ...
        f"щоб отримати наступний бонус 1₴. Переконайтеся, що на чеку є QR код. {consent}"
    )
...
def main_menu_keyboard() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup(
        ...
        resize_keyboard=True,
        input_field_placeholder="Надішліть фото чека з QR кодом або виберіть дію",
    )
...
def contact_saved_text() -> str:
    return (
        f"Номер збережено. Тепер можете надіслати фото чека, щоб отримати бонус 1₴. "
        f"Переконайтеся, що на чеку є QR код. {consent_notice()}"
    )
